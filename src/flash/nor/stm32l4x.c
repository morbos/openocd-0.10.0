...
static int stm32l4_probe(struct flash_bank *bank)
{
	...
	case 0x461:
		...
		break;
	case 0x495:
		max_flash_size_in_kb = 768;
		break;
	default:
		LOG_WARNING("Cannot identify target as a STM32L4 family (%x).", device_id);
		return ERROR_FAIL;
	}
...
static int get_stm32l4_info(struct flash_bank *bank, char *buf, int buf_size)
{
	...
	case 0x461:
		device_str = "STM32L49x";
		break;

	case 0x495:
		device_str = "STM32WB55";
		break;
		
	default:
		snprintf(buf, buf_size, "Cannot identify target as a STM32L4 (%x)\n", device_id);
		return ERROR_FAIL;
	}
